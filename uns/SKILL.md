...
- 用户要查看 Flow 状态（running/stopped）→ 走 `flow/list.md`
...
4. **history 参数复杂，必读文档** — 时间戳单位（秒/毫秒）、聚合参数极易出错，读 `history.md` 后再执行
...
| 意图 | 加载文件 | 风险 | 说明 |
|------|---------|------|------|
| 浏览命名空间树 | `browse.md` | — | 获取文件夹下的子节点列表、层级结构 |
| 读取数据点当前值 | `read.md` | — | 实时 VQT 值查询，支持通配符 |
| 写入数据点 | `write.md` | — | 发布数据，value 必须是对象 |
| 查询历史/时序数据 | **必读** `history.md` | — | 时间戳参数复杂，必读后执行 |
| 搜索节点 | `search.md` | — | 按关键字/前缀搜索 |
| 创建节点 | `create.md` | — | 新建 folder 或 thing 节点 |
| 更新节点元数据 | `update.md` | — | 修改字段定义或描述 |
| 删除节点 | `delete.md` | ⚠️ 硬删除不可逆 | 软删除可恢复，硬删除永久清除 |
| 恢复已删除节点 | `restore.md` | — | 撤销软删除 |
| 服务信息/健康检查 | `info.md` | — | 网关连通性验证 |
...
| 查历史趋势 | **先读 `history.md`**，再执行 | 不要循环 `read`（read 只返回最新值） |
